...
(define infile (open-input-file "png_zlib.rkt" #:mode 'binary))
...
(define ZI (new zlib_inflater%))

(define INFILE (open-input-file "op" #:mode 'binary))
(define data (read-bytes 66 INFILE))
(send ZI do_inflate data)
(set! data (read-bytes 66 INFILE))
(send ZI do_inflate data)
(set! data (read-bytes 20 INFILE))
(send ZI do_inflate data)
